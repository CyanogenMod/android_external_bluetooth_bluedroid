...
void bta_hd_api_enable(tBTA_HD_DATA *p_data)
{
    ...
    {
        APPL_TRACE_ERROR("%s: Failed to register HID device (%d)", __FUNCTION__, ret);
    }
...
void bta_hd_api_disable(void)
{
    ...
    {
        APPL_TRACE_ERROR("%s: Failed to deregister HID device (%s)", __FUNCTION__, ret);
    }
...
extern void bta_hd_connect_act(tBTA_HD_DATA *p_data)
{
    ...
    if (ret != HID_SUCCESS)
    {
        APPL_TRACE_WARNING("%s: HID_DevConnect returned %d", __FUNCTION__, ret);
    }
}
...
extern void bta_hd_disconnect_act(tBTA_HD_DATA *p_data)
{
    ...
    if (ret != HID_SUCCESS)
    {
        APPL_TRACE_WARNING("%s: HID_DevDisconnect returned %d", __FUNCTION__, ret);
    }
}
...
extern void bta_hd_report_error_act(tBTA_HD_DATA *p_data)
{
    ...
    APPL_TRACE_API("%s: error = %d", __FUNCTION__, p_report->error);

    ret = HID_DevReportError(p_report->error);

    if (ret != HID_SUCCESS)
    {
        APPL_TRACE_WARNING("%s: HID_DevReportError returned %d", __FUNCTION__, ret);
    }
}
...
extern void bta_hd_vc_unplug_act(tBTA_HD_DATA *p_data)
{
    ...
    if (ret != HID_SUCCESS)
    {
        APPL_TRACE_WARNING("%s: HID_DevVirtualCableUnplug returned %d", __FUNCTION__, ret);
    }
...
static void bta_hd_cback (BD_ADDR bd_addr, UINT8 event, UINT32 data, BT_HDR *pdata)
{
    ...
    APPL_TRACE_API("%s: event=%d", __FUNCTION__, event);
